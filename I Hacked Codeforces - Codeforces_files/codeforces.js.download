...
$.fn.autocompleteHandle = function() {
    return $(this).autocomplete(window.codeforcesOptions.suggestHandleLink, Codeforces.suggestHandleAutocompleteOptions);
};

$.fn.textcompleteHandle = function(options) {
    var handleAutocompleteCache = {};
    function filterRows(term, rows) {
        const result = [];
        for (const row of rows) {
            if (row && row[0].toLowerCase().indexOf(term) >= 0 ? row : null) {
                if (row[0].toLowerCase() === term) {
                    result.unshift(row);
                } else {
                    result.push(row);
                }
    ...
    return $(this).textcomplete([
        {
            match: /(^|\b)([a-zA-Z0-9_\-\.]{3,})$/,
            template: function(row) {
                return row[2];
            },
            search: function (term, callback) {
                term = term.toLowerCase();
                let rows = handleAutocompleteCache[term];

                if (rows) {
                    callback(filterRows(term, rows));
                    return;
                }

                if (term.length > 3) {
                    var termPrefix = term.substring(0, term.length - 1);

                    while (termPrefix.length >= 3) {
                        rows = handleAutocompleteCache[termPrefix];
                        if (rows) {
                            callback(filterRows(term, rows));
                            return;
                        }
                    ...
                    var termSuffix = term.substring(1);

                    while (termSuffix.length >= 3) {
                        words = handleAutocompleteCache[termSuffix];
                        if (words) {
                            callback(filterRows(term, words));
                            return;
                        }

                        termSuffix = termSuffix.substring(1);
                    }
                }

                $.post(window.codeforcesOptions.suggestHandleLink, {q: term}, function (data) {
                    if (data && typeof(data) === 'string') {
                        const rows = data.split(/\n/)
                            .filter(line => line.trim().length !== 0)
                            .map(line => line.split(/\|/));
                        handleAutocompleteCache[term] = rows;
                        callback(filterRows(term, rows));
                    } else {
                        callback({});
                    }
...
Codeforces = new function () {
    ...
    this.reloadAndShowMessageOrShowError = function (json, message) {
        if (json && json["success"] === "true") {
            ...
            if (json.hasOwnProperty("error") && json["error"]) {
                Codeforces.showMessage(json["error"]);
            }
        }
    }
    ...
    this.applyDatatableJs = function(clickMessage) {
        let $datatables = $(".datatable table");

        // --- Hide/show by Ctrl+Alt+Click ---
        $datatables.on("click", "th", function(e) {
            Codeforces.showMessage(clickMessage);
            ...
            const hidden = $th.data("hiddenCol") || false;

            $table.find("tr").each(function() {
                $(this).children().eq(colIndex).css("display", hidden ? "" : "none");
            });

            $th.data("hiddenCol", !hidden);
            return false;
        });
        ...
        $datatables.each(function() {
            ...
            $table.on("dragstart", "th", function(e) {
            ...
            $table.on("drop", "th", function(e) {
                e.preventDefault();
                const from = parseInt(e.originalEvent.dataTransfer.getData("colIndex"), 10);
                const to = $(this).index();
...
//            window.localStorage[key] = defaultValue;
    ...
    this.addUserTalkCopyHandler = function () {
        document.querySelectorAll('.talk-text').forEach(el => {
            el.addEventListener('copy', (e) => {
                const target = e.target.closest('.talk-text');
                if (!target) return;
                ...
                const hasUser = html.includes('talk-text-user');
                const hasSubmission = html.includes('talk-text-submission');
                if (!hasUser && !hasSubmission) return;

                container.querySelectorAll('a.talk-text-user').forEach(link => {
                    const handle = link.textContent.trim();
                    const textNode = document.createTextNode(`[user:${handle}]`);
                    link.replaceWith(textNode);
                });

                container.querySelectorAll('a.talk-text-submission').forEach(link => {
                    const submissionId = link.textContent.trim();
                    const textNode = document.createTextNode(`[submission:${submissionId}]`);
                    link.replaceWith(textNode);
                });
    ...
    this.copiedToClipboardText = "Copied to clipboard";

    this.setCopiedToClipboardText = function(copiedToClipboardText) {
        this.copiedToClipboardText = copiedToClipboardText;
    }

    this.copyTextToClipboard = function (text) {
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(text).then(
                () => Codeforces.showMessage(this.copiedToClipboardText),
                (err) => Codeforces.showMessage("Failed to copy: " + err)
            );
        } else {
            // Fallback: use execCommand (works in more places, but deprecated)
            ...
            try {
                ...
                Codeforces.showMessage("Failed to copy: " + err);
            }
            document.body.removeChild(textarea);
        }
    };

    this.listenCopyable = function ($scope = $(document)) {
        $scope.find(".copyable").click(function () {
            const text = $(this).attr("data-clipboard-text");
            if (text) {
                Codeforces.copyTextToClipboard(text);
            }
            return false;
        });
    }

    this.formatHandleSuggestion = function (row, position, numberOfItems, searchTerm) {
        return row[2];
    };

    this.highlightHandleSuggestion = function(html, term) {
        // split into ["<tag>", "text", "<tag>", "text", ...]
        const parts = html.split(/(<[^>]+>)/g);
        const rawText = parts.filter(part => !(part.startsWith("<") && part.endsWith(">"))).join("");
        const matchIndex = rawText.toLowerCase().indexOf(term.toLowerCase());
        let rawIndex = 0;
        let result = "";
        for (const part of parts) {
            if (part.startsWith("<") && part.endsWith(">")) {
                ...
                function addHighlighted() {
                    ...
                    result += "<u style='font-weight: bold !important;'>" + highlighted + "</u>";
                    highlighted = "";
                }
                for (let i = 0; i < part.length; i++, rawIndex++) {
                    if (matchIndex >= 0 && matchIndex <= rawIndex && rawIndex < matchIndex + term.length) {
                        highlighted += part.charAt(i);
                    } else {
                        addHighlighted();
    ...
    this.suggestHandleAutocompleteOptions = {
        delay: 200,
        width: 200,
        selectFirst: false,
        matchContains: true,
        minChars: 3,
        formatItem: this.formatHandleSuggestion,
        highlight: this.highlightHandleSuggestion
    }
};
